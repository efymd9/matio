...
The same signal surface drives the `/welcome` **self-hosted email-code fallback** (`EmailCodeSignIn`, when the one-click ticket can't mint): `await signIn.create({ identifier })` → `await signIn.emailCode.sendCode()` → `await signIn.emailCode.verifyCode({ code })` → `await signIn.finalize()` — each returns `{ error }`, and `finalize()` (not `setActive`) sets the active session. This replaced the Clerk `<SignInButton mode="modal">`, whose "Continue with Google" is hard-blocked inside FB/IG webviews (Google returns `disallowed_useragent`) — exactly the audience that flow serves.

### In-app browsers (FB/IG) break checkout + sign-in — detect and route around them

~60% of ad traffic lands in the Instagram/Facebook in-app browser (WKWebView/System WebView). They break three things at once: Google OAuth is blocked, the Embedded Checkout iframe + Apple/Google Pay are flaky, and the post-payment cookie/sign-in is unreliable. `lib/in-app-browser.ts` is a **pure UA matcher** (`FBAN|FBAV|FB_IAB|FB4A|Instagram`) imported BOTH server-side (force HOSTED Stripe in the checkout actions: `embedded = embeddedCheckoutEnabled() && !isInAppBrowser(ua)`) and client-side (the `OpenInBrowserHint` escape). Two gotchas: (1) the `embedded` flag must stay in the Stripe idempotency-variant digest so a webview's hosted session can't collide a same-token embedded one; (2) `OpenInBrowserHint` now also renders inside the server-rendered `/welcome` fallback, so its `navigator`-based detection uses `useSyncExternalStore` (server snapshot `false`, client `true`) to stay hydration-safe — a lazy `useState(navigator…)` initializer would mismatch on hydration there (it was fine on the paywall, which is a client-only end-state never in server HTML).
