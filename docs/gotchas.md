...
### `@mux/mux-video-react` is NOT the `<mux-video>` custom element — `envKey`/`disableTracking` are read once, at initialize; `disableCookies` is reactive since 0.31.3
...
- **`@mux/mux-video-react`** (the watch player) renders a **bare `<video>`**. `envKey` / `disableTracking` are read only inside playback-core's `initialize()`, from an effect keyed on the src alone (`dist/index.mjs`) — flipping them on a mounted element does nothing: no reload, no play(). **Since 0.31.3 (muxinc/elements#1349) `disableCookies` IS reactive**: its own effect calls playback-core's `applyDisableCookies`, which destroys the running monitor (`video.mux.destroy()` — final view-end beacon flushed, handle dropped) and re-runs `setupMux` with the CURRENT props on a microtask; the media element and hls.js are left alone, so playback never stalls. Because a consent flip changes all three props together, both directions now land on the spot: a withdrawal stops beaconing (the re-init sees `disableTracking=true` and installs nothing; the wrapper also expires `muxData`), a grant starts a monitor mid-episode — a `viewstart` with no preceding `viewend`, because nothing was running (before the grant `disableTracking=true` meant no monitor, and `applyDisableCookies` destroys only an existing `el.mux`). Before 0.31.3 a grant waited for the next initialize (auto-advance src swap, token-refresh remount, manual swap) and a mid-episode **withdrawal kept beaconing** — which is why the player still stops it itself with mux-embed's `video.mux.destroy()` (synchronous belt-and-braces; the wrapper checks `deleted` before its own destroy, so there is no double call) and the banner's `clearMarketingCookies()` expires `muxData`. The init-time rule that came with 0.31.3: a player mounted with `disableCookies` no longer deletes an existing `muxData` cookie (the element cannot tell "declined" from "not yet known"), so clearing on a real withdrawal is the app's job — ours is `clearMarketingCookies()`. Proven against the real wrapper + real mux-embed in `components/watch/player.test.tsx`.
...
### ESLint 10 removes the deprecated rule context — three plugins behind `eslint-config-next` have no release for it

ESLint 10 (2026-02) deleted `context.getFilename()`, `getSourceCode()`, `getPhysicalFilename()`, `getCwd()`, `parserOptions`, `parserPath` and `sourceCode.getJSDocComment()` / `isSpaceBetweenTokens()` / `getTokenOrComment{Before,After}()`. `eslint-config-next@16.3.4` declares `eslint >=9.0.0`, but it composes `eslint-plugin-react@7.37.5`, `eslint-plugin-jsx-a11y@6.10.2` and `eslint-plugin-import@2.32.0`, whose own peers stop at `^9` and whose code still calls the removed members. On ESLint 10 `pnpm lint` dies on the first file: `Error while loading rule 'react/display-name': contextOrFilename.getFilename is not a function` (`settings.react.version: "detect"` → `resolveBasedir(context)`). None of the three has an ESLint-10 release (react: jsx-eslint/eslint-plugin-react#3977 open since 2026-02, last release 2025-04; jsx-a11y: #1075 open, last release 2024-10; import: 2025-06 — `eslint-module-utils@2.14.0` added v10 support, the plugin itself did not), and `eslint-config-next@canary` still pins the same trio. pnpm installs them regardless (`strict-peer-dependencies` is off) — the `✕ unmet peer eslint@"… || ^9"` lines on `pnpm install` are this, not a broken install.

What we do (#157, part 2): `eslint.config.mjs` wraps exactly those three plugin objects in `@eslint/compat`'s `fixupPluginRules` — the ESLint team's own shim (peer `^8.40||9||10`), which hands their rules an `Object.create(context)` carrying the removed members again. Rule set and levels are unchanged (113 rules, byte-identical `--print-config` apart from the note below). The native plugins (`react-hooks`, `@next/next`, typescript-eslint, storybook) are deliberately NOT wrapped — they already use `context.sourceCode`. The shim leaves with the plugins' native releases — registry row. Two smaller v10 facts met on the way: `--print-config` now prints each rule's `meta.defaultOptions` merged into its options (`no-unused-vars: [0]` → `[0, {…defaults}]` — a serialisation change, the level is the same), and the new config-lookup-from-file algorithm still honours the root `globalIgnores([".claude/worktrees/**"])` — a nested worktree with its own `eslint.config.mjs` is skipped, verified with a probe copy.
