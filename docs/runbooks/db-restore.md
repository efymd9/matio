...
- **GDPR**: восстановление возвращает и то, что было удалено по запросу
  ...
  без основания. Механика: аккаунт в Clerk уже удалён, сам по себе вебхук
  второй раз не придёт — переотправить событие `user.deleted` из журнала
  эндпойнта в дашборде Clerk (Webhooks → endpoint → Messages → Resend;
  обработчик идемпотентен), и только если журнал уже не хранит сообщение —
  те же операторы руками (`DELETE FROM show_reminders WHERE email = …`,
