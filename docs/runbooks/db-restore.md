...
- **GDPR**: восстановление возвращает и то, что было удалено по запросу
  ...
  без основания. Механика (#180): по реестру заявок в
  `docs/runbooks/gdpr-requests.md` §6 — для каждой строки со статусом
  `erased`, исполненной ПОЗЖЕ даты дампа:

  ```bash
  DATABASE_URL='<восстановленная база>' pnpm erase-user <user_…>           # dry-run: что вернулось
  DATABASE_URL='<восстановленная база>' pnpm erase-user <user_…> --apply   # стереть заново
  ```

  Тот же код, что у вебхука `user.deleted` (`lib/erase-user.ts`),
  идемпотентно. Stripe и PostHog уже стёрты в первый раз — вендорские ключи
