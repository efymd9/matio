...
**Authorized parties (`azp` check, #100)**: `clerkMiddleware` in `proxy.ts` verifies the session token's `azp` claim (the browser origin the token was minted for) against an allowlist, so a token minted for any other origin on the same Clerk instance is refused by the web AND by `/api/v1`. The list is not hard-coded — `lib/authorized-parties.ts:resolveAuthorizedParties(process.env)` builds it at deploy from `VERCEL_PROJECT_PRODUCTION_URL` (Vercel's own variable: the project's shortest production domain) plus `NEXT_PUBLIC_APP_URL` when it is an https origin, and applies it only where `VERCEL_ENV === "production"`. What is authorized where: **prod** → `https://matio.tv` + `https://www.matio.tv` (a custom domain gets its `www.` twin); **staging** → `https://matio-staging.vercel.app` (the bench is the production deployment of its own project, so it qualifies; no www twin for a `*.vercel.app` host); **Vercel previews and localhost → no list at all** (`undefined` = Clerk verifies exactly as before) — preview URLs are minted per commit and cannot be enumerated, and a fixed list would sign the reviewer out of every preview. Garbage in either variable degrades to "no list", never to "nobody". **Native tokens**: `@clerk/backend` ≥3.x (3.17.1 installed) rejects a token with NO `azp` when a list is set — and the app's `@clerk/expo` Bearer tokens carry none (no browser Origin at mint) — so the per-request options callback withholds the list from a `Bearer` token whose payload has no `azp` (`authorizedPartiesForRequest`); such a token still passes the signature/expiry/issuer checks, and a browser-minted token (always with `azp`) replayed as a Bearer is still held to the list. A refused token is a signed-out request (no handshake): admin routes bounce to sign-in, `/api/v1` Bearer routes answer 401. Adding a domain to a project changes the list on the next deploy; nothing to configure in the Clerk dashboard.
