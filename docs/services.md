...
**`user.deleted` = our art. 17 GDPR erasure.** Deleting the account in Clerk (UserProfile → "Delete account", the dashboard, or an owner-run erasure request — always Clerk, never SQL) is the one trigger; the handler runs `DELETE FROM users WHERE id = <clerk id>` and the FK actions in `db/schema/*` take the rest (CASCADE: `subscriptions`, `watch_progress`, `watch_days`; SET NULL: `trial_sessions`, `visitors`, `marketing_links.created_by`), plus an explicit delete of `show_reminders` rows for the account's address (that table keys on the email, not the account). Idempotent: a redelivery for an already-erased user is a 200 no-op. Two things happen BEFORE the `users` DELETE (#164): a live Stripe subscription is set to `cancel_at_period_end: true` (best-effort — a Stripe failure is logged and sent to Sentry by id, never the address, and the erasure still completes; the cancel request is repeated harmlessly on a retry), and the account's `stripe_customer_id` is written to `erased_customers` — the tombstone that stops the customer's later Stripe webhooks from re-creating the account through `claimGuestCheckout` (see [Stripe → account erasure](#stripe-subscriptions)). The tombstone write is NOT best-effort: if it fails, the handler 500s and Clerk retries, because without it the erasure is reversible. Full map: the `/gdpr` skill → `references/data-map.md`.
...
**Authorized parties (`azp` check, #100)**: `clerkMiddleware` in `proxy.ts` verifies the session token's `azp` claim (the browser origin the token was minted for) against an allowlist, so a token minted for any other origin on the same Clerk instance is refused by the web AND by `/api/v1`. `lib/authorized-parties.ts:resolveAuthorizedParties(process.env)` builds the list at deploy from `VERCEL_PROJECT_PRODUCTION_URL` (Vercel's own variable: the project's shortest production domain) plus `NEXT_PUBLIC_APP_URL` when it is an https origin, plus the hard-pinned `SITE_URL` (`lib/seo.ts`) — and applies it only where `VERCEL_ENV === "production"`. What is authorized where: **prod** → `https://matio.tv` + `https://www.matio.tv` (a custom domain gets its `www.` twin); **staging** → `https://matio-staging.vercel.app` plus the same two matio.tv origins (the bench is the production deployment of its own project, so it qualifies; no www twin for a `*.vercel.app` host; the apex is a harmless extra there — a matio.tv token is signed by the other instance); **Vercel previews and localhost → no list at all** (`undefined` = Clerk verifies exactly as before) — preview URLs are minted per commit and cannot be enumerated, and a fixed list would sign the reviewer out of every preview. Garbage in a variable is skipped; `SITE_URL` is pinned in because a VALID but wrong value (the system variable naming the legacy `matio-ten.vercel.app` alias, say) would otherwise yield a list without the apex, and `TokenInvalidAuthorizedParties` is not a handshake reason — every prod request would be signed out. **Native tokens**: `@clerk/backend` ≥3.x (3.17.1 installed) rejects a token with NO `azp` when a list is set — and the app's `@clerk/expo` Bearer tokens carry none (no browser Origin at mint) — so on **`/api/v1/*` only** the per-request options callback withholds the list when the header token Clerk is about to verify has no `azp` (`authorizedPartiesForRequest`); such a token still passes the signature/expiry/issuer checks, and a browser-minted token (always with `azp`) replayed as a Bearer is still held to the list. The header is read exactly as Clerk's `parseAuthorizationHeader` reads it (case-sensitive `Bearer`, a bare value is the token, any other scheme is none): when Clerk finds a header token it verifies that and never the cookie, so withholding the list on a header token can never relax the check on a cookie session — a looser reading would. Everywhere else (`/`, `/watch`, `/admin`, the web's own `/api/*`) the list always applies. A refused token is a signed-out request (no handshake): admin routes bounce to sign-in, `/api/v1` Bearer routes answer 401. Adding a domain to a project changes the list on the next deploy; nothing to configure in the Clerk dashboard.
...
**Localization**: `@clerk/localizations@^4.6.7` provides per-locale string bundles. `app/layout.tsx` reads the site locale via `getDict()` and passes the matching bundle (`enUS` default since 2026-07-04; `esES` when negotiation / geo affinity / the cookie resolves Spanish) to `ClerkProvider`'s `localization` prop. Every Clerk-rendered surface — sign-in/sign-up modal, UserButton dropdown, validation copy — picks it up. Locale changes via the in-header switcher propagate to Clerk's UI on the next `router.refresh` tick (a few hundred ms after the optimistic site dictionary flip — see [gotchas → optimistic locale state](./gotchas.md#optimistic-locale-state-not-just-context)). Adding a new locale = add another entry to the `CLERK_LOCALIZATIONS` map keyed on the site's `Locale` type.

**Branding (dashboard only, free plan — set 23.09.2026, #276)**: Application settings → name **Matio** (was `matio`: Clerk's default emails print it three times and it names the email sender), logo **and** favicon = `app/icon.png` (the gold M on black — legible on the white default email and on the dark portal alike; the gold wordmark is not, on white), support email `contact@matio.tv` (shown by Clerk components and `{{support_email}}` in emails). Customization → Avatars: marble background `#E6B366` with an `#241205` silhouette (was Clerk purple + white) — the default avatar in the site's user menu. Account Portal → Customization: colour mode **Dark**, dark primary `#E6B366`, dark background `#0F0A07` (`accounts.matio.tv` is where a signed-out `/subscribe` and the admin sign-in land). Known limit: the portal takes no primary-*foreground* colour, so its primary buttons print white on gold (≈1.9:1) — the site's own Clerk modals are unaffected, `ClerkProvider` sets `colorPrimaryForeground`. Still Clerk's on the free plan: the layout and copy of every email and the «Secured by Clerk» mark — both are Pro. The eight Matio email templates are built and waiting in `infra/clerk/email-templates/` (not applied — see its README).
...
**Account erasure (Clerk `user.deleted` → Stripe, #164)**: the erasure handler (`app/api/webhooks/clerk/route.ts`) makes exactly one Stripe call — `subscriptions.update(<id>, { cancel_at_period_end: true })` on the account's live subscription (5s request timeout; idempotent at Stripe, so a Clerk redelivery re-sets the same flag). The viewer keeps the period they paid for; nothing is charged after it. The Customer object (email, billing address, invoices) is deliberately left in place — invoices are Stripe's own tax records either way (art. 17(3)(b)), and whether to `customers.del` the rest is an owner decision tracked in `docs/registry.md`. What makes the erasure stick on the Stripe side is the `erased_customers` tombstone: every later webhook for that customer (the cancel-at-period-end update, `customer.subscription.deleted` at period end, anything else) reaches `mirrorSubscription` with no local user, and a guest (`metadata.guest = "1"`) subscription would otherwise be re-claimed into a fresh Clerk user + `users` row — `isErasedCustomer` (`lib/guest-checkout.ts`) is asked first, in the mirror AND on `/welcome`, and the event is consumed with a warn by id. Nothing about this branches on `PAYMENTS_ENABLED`.
...
**Identity plumbing**: `_fbp`/`_fbc` are set by the browser pixel; `_fbc` is also derived from `?fbclid` in `proxy.ts` (consent-gated). At checkout, `startCheckout` snapshots `_fbp`/`_fbc`/IP/UA into Stripe `subscription_data.metadata` (the `capi_*` keys, incl. a `capi_consent` sentinel) — the same channel UTM attribution uses — so the context-less webhook can match the Purchase event. See `lib/capi-identity.ts`. **The snapshot does not outlive the event** (#165): right after the Purchase moment `mirrorSubscription` erases `capi_fbp`/`capi_fbc`/`capi_ip`/`capi_ua` from the subscription's metadata (`subscriptions.update` with empty strings — Stripe deletes a key set to `""`; `capi_consent` stays). Best-effort like CAPI itself: a Stripe failure is logged by subscription id and the next event whose payload still carries the keys retries; a sub still `incomplete` keeps them until it either grants access or expires. Note the erase itself produces one extra `customer.subscription.updated` delivery per purchase — a plain idempotent re-mirror. Subscriptions from before this shipped: `STRIPE_SECRET_KEY=sk_… pnpm stripe:scrub-capi` (lists ids + key names; dry-run by default, `--apply` to erase; deliberately never reads `.env.local`).
