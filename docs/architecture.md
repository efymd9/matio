...
| `erased_customers` | Art. 17 tombstones (#164) | `stripe_customer_id` (PK), `erased_at`. Written by the Clerk `user.deleted` handler BEFORE `DELETE FROM users`, write-if-absent. No FK on purpose (the referenced row is the one being deleted) and never pruned. `claimGuestCheckout`'s callers (`mirrorSubscription`, `/welcome`) check it first so a later Stripe webhook for the customer cannot re-create the account |
| `guest_checkout_sessions` | The guest half of "one open Checkout Session per buyer" (#224, [ADR 0002](./adr/0002-single-open-checkout-session.md)) | `claim_token_hash` (PK — HMAC of the `checkout_claim` cookie, never the value), `session_id` (the guest's last Checkout Session), `created_at` (reset on every claim). A pay-first guest has no Stripe customer to `list` sessions by, so `claimSoleGuestSession` swaps the new id in and hands the old one back in ONE statement (`INSERT … ON CONFLICT DO UPDATE … RETURNING old.session_id`, Postgres 18) and the action expires the old one by id. Self-pruned after 24h — a Checkout Session's own maximum life |
...
| `episodes` | Episode with Mux linkage | `mux_asset_id`, `mux_playback_id`, `mux_playback_policy` (`public`\|`signed`), `status` (`processing`\|`ready`\|`errored`), `intro_start_seconds` + `intro_end_seconds` (nullable; drive the "Skip intro" chip), `(season_id, number)` unique. Branching (#143, [ADR 0001](./adr/0001-branching-video-graph.md)): `branch_of_episode_id` (self-FK, `SET NULL`; NOT NULL = a branch — reachable only through a choice, hidden from every public list, numbered 900+ by convention), `fork_prompt_en` / `fork_prompt_es` (viewer copy, site locales), `fork_window_seconds` (3–30, default 10 — seconds before the end the prompt appears = the timer) |
| `episode_choices` | Edges of the branching graph | `from_episode_id` (CASCADE), `to_episode_id` (RESTRICT — remove the choice before deleting its target), `position`, `label_en` / `label_es`, `is_default`; unique `(from, position)`, partial unique `(from) WHERE is_default`. The SIZE of an episode's edge set is its meaning (`lib/branching.ts:resolveNextStep`): ≥2 = fork (prompt shown, targets must be branches OF that episode), 1 = silent auto-transition (reconvergence; any ready episode), 0 on a branch = ending, 0 on a regular episode = linear `episodes[idx + 1]`. The admin form (`validateForkDraft`) and the publish guard (`validateBranchGraph`, DFS from every listed ready episode: unfinished fork / not-ready target / non-branch fork target / cycle) are pure and table-tested. The player (#144) resolves the same rules against the episodes on the page (`resolveCandidates` → fork / next / end), prefetches a token per candidate, shows the prompt (`components/watch/fork-choice-overlay.tsx`) for `fork_window_seconds` on the parent's own clock, and takes the pick — or the default at `ended` — down the gapless auto-advance path (same `<video>`, new playbackId + token) |
...
FK cascade defaults: deleting a `show` removes its `seasons` → `episodes` → `trial_sessions` (via FK CASCADE). `users.id` cascades into `subscriptions` and `watch_progress`; nulls out `trial_sessions.user_id`. Deleting an episode cascades its outgoing `episode_choices` and nulls `branch_of_episode_id` on its branches, but is REFUSED while another episode's choice points at it (RESTRICT) — the admin episode page hides the delete button for such rows.
...
- `users` table mirrors Clerk via `user.created` webhook (`app/api/webhooks/clerk/route.ts`) — handler is idempotent (`onConflictDoNothing` on `users.id`). The same handler answers `user.deleted` with `DELETE FROM users` (art. 17 erasure: FK cascades + an explicit `show_reminders` delete by the account's address; a missing row is a 200 no-op, so redeliveries are safe), after setting a live Stripe subscription to `cancel_at_period_end` (best-effort) and tombstoning the Stripe customer id in `erased_customers` (not best-effort — the tombstone is what keeps a guest-checkout webhook from re-creating the account, see [services → Stripe](./services.md#stripe-subscriptions)). Deleting the account in Clerk is the ONLY erasure mechanism — a manual GDPR request is executed there, never by SQL.
...
1. **Visiting `/watch/<slug>`** (`app/watch/[showSlug]/page.tsx`):
   ...
   - Else: read the `trial_session` cookie (may be absent). `findTrialSession(cookie, show.id)` is a **read-only** lookup that returns the row if one exists for this (cookie, show).
     ...
     - Row found and active, OR no row yet → render Player in `trial` mode. The trial 60-second clock begins when the player requests its first token — which, since autoplay-on-land (2026-06-09), happens on mount for human visitors (the page passes `autoplay={!userAgent().isBot}`; crawlers keep the poster play-gate and never mint).
...
2. **Player mounts** (`components/watch/player.tsx`):
   - Outer `Player` picks the initial episode; the inner `EpisodePlayback` (keyed on `mountKey` — bumped by manual swaps only) does the actual playback work.
...
- `components/watch/player.tsx` — split into two components in one file:
  - **`Player` (outer)** — owns state that should survive episode swaps: `currentEpisodeId`, a `mountKey`, overlay visibility, lock toggle, and two transition callbacks: `swap` (manual selection — bumps `mountKey`) and `advance` (auto-advance at `ended` — does NOT bump it).
  - **`EpisodePlayback` (inner, `key={mountKey}`)** — owns per-episode state: the `playback` snapshot (`{episodeId, playbackId, token, expiresAt}` — the three travel together because a Mux JWT is signed per playback id), paywall, aspect ratio, captions detection, skip-intro chip, last-saved position. **Manual swaps** remount the inner via the key bump, so per-episode state resets to its initial value naturally — the pattern required by React 19's `react-hooks/set-state-in-effect` rule (see [gotchas → React 19 hooks rules](./gotchas.md#react-19-hooks-rules)). **Auto-advance** keeps the component (and its `<video>` element) alive and resets state explicitly: refs in the `onEnded` handler, visual state in a render-phase `prevEpisodeId` adjustment (React's "adjust state when props change" pattern — allowed where effect-body setState is not; `Date.now()` is also banned in render by `react-hooks/purity`, which is why the prefetch-expiry decision lives in the handler). Element identity is load-bearing: WebKit's autoplay blessing is per-element and survives src changes but not remounts.
...
3. The outer bumps `mountKey` → the inner `EpisodePlayback` unmounts and remounts. The playback snapshot, paywall flag, aspect ratio, caption detection, skip-intro chip, and last-saved position all start fresh — no manual resets in effect bodies.
4. The new inner's token-fetch effect fires on mount → pulls a JWT for the new episode. During the fetch the inner renders the "Loading" splash (`playback === null`).
5. Subscriber: cross-session resume from `watch_progress` only applies to the episode the mount started on (the outer snapshots `initialEpisodeId` in state — the prop changes after `router.replace(?ep=…)` re-renders the page and must not re-aim resume mid-playback); subsequent swaps start from 0 by design.
6. Trial: `trial_sessions` is keyed at the show level, so any episode in the show plays during the active trial window.

The downside of the keyed-remount pattern is a brief Loading splash on every manual swap (the `<MediaController>` + `<MuxVideo>` are torn down and rebuilt). The trade-off is that React 19's hook-rule lint passes cleanly and the per-episode state model stays simple. Auto-advance is the exception and pays the explicit-reset cost on purpose — see below.

**Instant auto-advance (replaces the Up Next countdown for subscriber/member/free):** the `<MuxVideo onEnded>` handler saves completion, then advances to an unlocked next episode immediately on the **same `<video>` element** — `playbackId`+`token` change together, which `@mux/mux-video-react` applies as an in-place teardown + re-init (it ignores tokens-only changes, hence `key={refreshNonce}` for subscriber token refreshes — the one remaining live remount). `autoPlay="any"` restarts playback on the still-blessed element (unmuted where the user has interacted or the element was gesture-blessed; muted fallback → "Tap for sound" pill). To make the cut instant: the next episode's token is **prefetched** when the playhead crosses `duration - PRELOAD_LEAD_SECONDS` (45s), and a hidden muted `display:none` `<MuxVideo>` preloader buffers ~30s of the next stream — its segment fetches land in the browser HTTP cache (Mux segment URLs are deterministic per signed-URL expiry window and cacheable for a week; playlists are no-store but tiny), so the visible player's re-init starts in a few hundred ms. Prefetch failures degrade to fetch-on-advance under a dim overlay (still no remount). A transient "Up next · S·E — title" chip orients the viewer after the cut. The **legacy 60s trial keeps the countdown card** (`UpNextOverlay`, 7s auto-advance): its token TTL is the preview's remaining seconds, so a prefetched token would be dead on arrival, and the preview ends at the paywall anyway. Auto-advance into a **locked** episode keeps the remount path: the swap renders the SignupWall/Paywall full-surface with `?ep=<locked id>` in the URL for the post-signup redirect.
...
Accept-Language header
   │
   ├─ missing/empty ───────────────────────► en (DEFAULT_LOCALE — English
   │                                           since 2026-07-04, was es.
   │                                           Crawlers: Googlebot crawls
   │                                           from US IPs with NO
   │                                           Accept-Language, so this arm
   │                                           IS the site's indexed
   │                                           language; geo deliberately
   │                                           NOT consulted)
...
Underneath every matched route, `clerkMiddleware` verifies the session token's `azp` (the origin it was minted for) against `authorizedParties` (#100), so a token minted for another origin on the same Clerk instance is refused by the web and by `/api/v1` alike. The list is built at deploy from the environment by `lib/authorized-parties.ts` (`VERCEL_PROJECT_PRODUCTION_URL` + `NEXT_PUBLIC_APP_URL`, plus the hard-pinned `SITE_URL` so prod can never end up with a list that lacks its own apex; only on `VERCEL_ENV=production` deployments: prod → `https://matio.tv` + `https://www.matio.tv`, staging → `https://matio-staging.vercel.app` + that pair) and is **absent on Vercel previews and localhost** — preview URLs are per-commit and cannot be enumerated, and `undefined` means "verify as before", never "nobody". The app's native Bearer tokens carry no `azp` (and the installed `@clerk/backend` would reject them against any list), so on `/api/v1/*` only the per-request options callback withholds the list from an azp-less header token — read exactly as Clerk's own parser reads it, because when Clerk finds a header token it verifies that and never the cookie; a browser-minted token replayed as a Bearer still carries its `azp` and is still held to the list.
