...
Underneath every matched route, `clerkMiddleware` verifies the session token's `azp` (the origin it was minted for) against `authorizedParties` (#100), so a token minted for another origin on the same Clerk instance is refused by the web and by `/api/v1` alike. The list is built at deploy from the environment by `lib/authorized-parties.ts` (`VERCEL_PROJECT_PRODUCTION_URL` + `NEXT_PUBLIC_APP_URL`, only on `VERCEL_ENV=production` deployments: prod → `https://matio.tv` + `https://www.matio.tv`, staging → `https://matio-staging.vercel.app`) and is **absent on Vercel previews and localhost** — preview URLs are per-commit and cannot be enumerated, and `undefined` means "verify as before", never "nobody". The app's native Bearer tokens carry no `azp` (and the installed `@clerk/backend` would reject them against any list), so the per-request options callback withholds the list from an azp-less Bearer token; a browser-minted token replayed as a Bearer still carries its `azp` and is still held to the list.
