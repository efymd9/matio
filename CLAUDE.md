...
- **Auth gating**: `proxy.ts` sends unauth'd `/subscribe(.*)` requests to Clerk's **sign-up** page (not sign-in) — most paywall conversions are first-time users; Clerk's sign-up page still links to sign-in for the minority case. Admin routes keep `redirectToSignIn` since admins already have accounts. **Authorized parties (#100)**: `clerkMiddleware` gets `authorizedParties` from `lib/authorized-parties.ts` — built at deploy from the environment (`VERCEL_PROJECT_PRODUCTION_URL` + `NEXT_PUBLIC_APP_URL`), applied only on `VERCEL_ENV === "production"` deployments: prod → `https://matio.tv` + `https://www.matio.tv`, staging → `https://matio-staging.vercel.app`, **Vercel previews and localhost → `undefined`** (preview URLs are per-commit and cannot be listed; a fixed list would sign the reviewer out). Garbage in the variables degrades to "no list", never to "nobody". The installed `@clerk/backend` (3.17.1) rejects a token with NO `azp` when a list is set, so the per-request options callback withholds the list from a `Bearer` token whose payload carries no `azp` — the app's native `@clerk/expo` tokens — while a browser-minted token (always with `azp`) replayed as a Bearer is still held to it. Never hard-code the list, and never apply it on previews.
