...
- [docs/gotchas.md](./docs/gotchas.md) — **read this before touching webhooks or Mux** — version-specific traps for Next 16, Clerk 7, Stripe SDK 22 (API 2024+), Mux 15, Vercel Blob, shadcn/Base UI, Drizzle 0.45, tsx scripts
...
# config ignores mobile/**. Tracked in docs/registry.md; needs the owner's
# go-ahead for eslint-config-expo (a new dependency) — asked in PR #96's thread.
```
...
- **A bare worktree has no gitignored files**: no `node_modules`, no
  ...
  (the main checkout, `~/dev/matio`), then `pnpm install`, and `cd mobile &&
  npm ci` — without it the `mobile` vitest project fails the whole `pnpm
  test:unit` with `[TSCONFIG_ERROR] Failed to load tsconfig
  'expo/tsconfig.base'`. Do not invent env values, and do not commit them.
...
- **Mutations are idempotent.** A client retry — double tap, lost response,
  ...
  deterministic Stripe idempotency key where the params cannot drift
  (`customers.create` → `customer:<userId>`), **an expire-sweep where they
  can** — BOTH checkout flows create their session with NO key and then
  expire the buyer's other open session(s): the signed-in one by
  `checkout.sessions.list({customer})` (`createSoleOpenSession`, #217), the
  guest one by the id remembered under the claim cookie's HMAC in
  `guest_checkout_sessions` (`claimSoleGuestSession`, #224 — one `INSERT …
  ON CONFLICT DO UPDATE … RETURNING old.session_id`, so two parallel claims
  can never both read "nothing to expire"); a replayed key would hand back
  the cached body of a session the sweep has since killed — and a
  claim-stamp under `FOR UPDATE SKIP LOCKED` for batch
  dispatch (`show_reminders.notified_at`). A new write path must be able to
  answer, in one sentence, what makes running it twice safe.
...
- **Runner: Vitest** (`vitest.config.mts`). Default environment is `node`; a
  ...
  modules `vi.mock()`ed, the component tree real). `mobile/**` stays out of
  coverage, so diff-cover does not count mobile code («No lines with coverage
  information») — the guarantee there is the test itself, not a percentage.
  It resolves packages from
...
- The hero `MuxPlayer` on `/` is `next/dynamic({ ssr: false })` — keep it that way (a static import pulls ~350KB gzipped into every cold home-page visit) — **and keep its `key={muxDataEnabled ? "mux-data-on" : "mux-data-off"}`**: flipping `disable-tracking`/`env-key` on a live element makes @mux/mux-video run `unload(); …then(() => play())` with no `.catch`, and the orphaned play() lands in Sentry as an unhandled AbortError (issue #126, seen in prod 2026-08-24). This is the OPPOSITE rule to the watch player, where a remount kills WebKit's per-element autoplay blessing (`key` forbidden there) — and freezing consent into a mount-time snapshot is forbidden everywhere (the AUDIT.md H2 pre-consent leak). **The watch player is a different library and needs no remount** (#127): `@mux/mux-video-react` renders a bare `<video>`, so a consent flip there never touches the stream. Since 0.31.3 (muxinc/elements#1349, in via #242) the wrapper re-attaches Mux Data itself when `disableCookies` changes — mux-embed `destroy()` on the running monitor, then a fresh `setupMux` with the CURRENT props on a microtask, media and hls.js untouched — so a withdrawal stops beacons on the spot and a grant starts the monitor mid-episode (before 0.31.3 the props were read only at `initialize()`: a grant waited for the next auto-advance / token refresh / manual swap and a withdrawal kept beaconing — which is why `components/watch/player.tsx` still calls `video.mux.destroy()` on withdrawal as the synchronous belt-and-braces) and `clearMarketingCookies` expires `muxData` (the wrapper clears it too, but no longer deletes one at init — by design). Both contracts are pinned by `components/watch/player.test.tsx` against the real wrapper; see [gotchas → Mux](./docs/gotchas.md#mux-sdk-14). **The hero's `onError` reacts to FATAL errors only** (#128): `<mux-player>` forwards every playback-core `error` — recoverable ones included (`detail.fatal === false`, e.g. the "Attempting to reconnect…" MediaError) — and the player's own handler drops the non-fatal ones, so ours does too. A fatal error puts the backdrop back (`videoPlaying` reset) and unmounts the player; the one fatal error we fix ourselves is the 60s preview token running out under the looping teaser (JWT `exp` read client-side — that also covers Safari's native HLS, where playback-core attaches no `NETWORK_TOKEN_EXPIRED` code): the hero fetches `/api/hero-preview-token` (the same featured-show / first-episode / 60s mint the page embeds — `lib/hero-preview.ts`) and remounts with it, at most 20 cycles per page, then rests on the backdrop. Nothing on that path throws or logs — no new Sentry source.
...
app/
  ...
  admin/                   # /admin — admin role required (proxy-gated, DB check)
    analytics/             # /admin/analytics — the spec'd retention dashboard
                           #   (page.tsx, 7 blocks — see "Analytics dashboard
                           #   v2" rule) in BOTH modes; legacy-dashboard.tsx
                           #   (pre-2026-07-18, MRR/subs) only via
                           #   ?view=legacy in paid mode
  ...
  api/
    v1/                    # JSON surface for the mobile app (Expo). Native
      ...
      progress/            #   POST — the app's watch-progress save; the SAME
                           #     write as the web action (lib/watch-progress.ts),
                           #     Bearer-only (401 anonymous, nothing written)
      continue/            #   GET — continue-watching rail; getContinueWatching()
                           #     verbatim + absolute media URLs, Bearer-only;
                           #     branching shows filtered out (linearShowsOnly)
      watch-segments/      #   POST — the app's retention-bucket flush; the SAME
                           #     write as the web action (lib/watch-segments-
                           #     write.ts); Bearer OR device-id, positional gate
    admin/
    ...
    webhooks/
      clerk/               # user.created → mirrors to users; user.deleted →
                           ...
                           #   (best-effort) + EVERY customer id into
                           #   erased_customers — the row's and what
                           #   customers.search finds by address (#223)
                           #   (tombstone — #164); after
  ...
  subscribe/               # /subscribe — single membership card ($25/mo,
                           #   charged today); CTA links to /checkout
...
components/
  ...
  site/                    # header, footer, cookie-banner, language switcher,
                           ...
                           #   complete-registration-pixel, purchase-pixel
                           #   (ChatGPT Ads subscription_created on the
                           #   Stripe-verified checkout return — paid mode),
...
db/
  ...
  schema/                  # one file per domain; re-exported from schema/index.ts
                           ...
                           #   erased_customers (art. 17 tombstones: Stripe
                           #   customer ids of erased accounts; no FK on
                           #   purpose, never pruned — the claim path checks
                           #   it before re-creating an account)
                           #   episode_choices (branching-video edges; a
                           #   branch itself is an episodes row with
                           #   branch_of_episode_id — see "Branching video")
                           #   guest_checkout_sessions (#224: HMAC of the
                           #   guest's checkout_claim cookie → the id of
                           #   their last Checkout Session, so the guest
                           #   sweep can expire it by id; self-pruned 24h)
...
lib/
  ...
  authorized-parties.ts    # universal + PURE: Clerk `authorizedParties` from
                           #   the env (prod + staging only; previews/local →
                           #   undefined) + the per-request exemption for
                           #   azp-less native Bearer tokens — see "Auth gating"
  ...
  guest-checkout-sessions.ts # server-only, db injected: hashClaimToken +
                           #   claimSoleGuestSession — the guest half of the
                           #   one-open-session sweep (#224): one upsert
                           #   RETURNING old.session_id, 24h self-prune
  ...
  watch-segments.ts        # 10s bucket constants (universal; player + action +
                           #   the app via lib/api/types.ts re-export)
  watch-segments-write.ts  # server-only saveWatchSegmentsFor — the ONE
                           #   retention-counter write (timeline bound, dedupe,
                           #   (episode,day,bucket) upsert, anonymous session-
                           #   row proof + positional gate, UPDATE-only
                           #   total_watched credit) behind app/watch/actions.ts
                           #   AND /api/v1/watch-segments; never duplicate it
  watch-progress.ts        # server-only saveWatchProgressForUser — the ONE
                           #   progress write (clamp, GREATEST max_position,
                           #   live completed, watch_days tick, paid-mode
                           #   ownership gate) behind app/watch/actions.ts
                           #   AND /api/v1/progress; never duplicate it
  ...
  capi-identity.ts         # server-only _fbp/_fbc/IP/UA capture + Stripe
                           ...
                           #   + CAPI_IDENTITY_SCRUB: the post-Purchase erase
                           #   patch the mirror sends to Stripe (#165)
  ...
  erase-user.ts            # universal (no server-only — scripts/erase-user.ts
                           ...
                           #   cancel → customers.search by address →
                           #   tombstone ALL ids → DELETE reminders → DELETE
  ...
  i18n/                    # dictionaries.ts + server.ts + client.tsx (optimistic
                           ...
                           # + app-dictionaries.ts: APP-ONLY es/en copy (sign-in
                           #   steps, update wall; universal, bundled by Metro,
                           #   not by the web — the admin-dictionaries precedent)
...
mobile/                    # Expo app (npm, OUTSIDE the pnpm workspace).
                           #   src/app/(tabs)/ = Home · Browse · Account ·
                           #   Settings under the floating glass tab bar
                           #   (components/glass-tab-bar.tsx); show/[slug],
                           #   watch/[episodeId], sign-in = root-stack
                           #   screens, pushed OVER the tabs (no bar) —
                           #   see the "Mobile app" rule (#245). Home =
                           #   carousel + the hero-card feed (components/
                           #   home-feed.tsx; rules = lib/home-feed.ts, #248)
...
scripts/
  ...
  stripe-setup.ts          # pnpm stripe:setup — "Matio Membership" $25/mo
                           #   product+price (STRIPE_PRICE_MONTHLY). Archives
                           #   stale prices per plan on amount mismatch.
  scrub-capi-metadata.ts   # pnpm stripe:scrub-capi — one-shot erase of the
...
- **Mobile app + `/api/v1` (Expo/React Native, `mobile/`)**: the native client talks to `/api/v1/*` — `config` (startup: kill-switch flags, `minSupportedBuild` retirement lever, signup gate, legal URLs; DB-free), `catalog`, `shows/[slug]`, `playback-token`, and — since #96 — `progress` (POST, Bearer-only: the web's `saveWatchProgress` core in `lib/watch-progress.ts`, so clamp / `GREATEST` max-position / live `completed` / `watch_days` cannot drift between surfaces; anonymous = 401 + nothing written) and `continue` (GET, Bearer-only: `getContinueWatching()` verbatim with additive `positionSeconds` / `durationSeconds` / `episodeTitle` / `show.orientation`, minus branching shows — #288). The app reads copy from `lib/i18n/dictionaries.ts` + the app-only `lib/i18n/app-dictionaries.ts` via `mobile/src/shared/i18n.ts` (device language detected with NO new dependency — `I18nManager` constants → Hermes `Intl` → `pickFromLanguageTags`; choice persisted in SecureStore; since #245 the switcher is the Language group of the Settings tab — the home header carries no pill), saves progress from `mobile/src/watch/use-progress-saver.ts` (10s while playing, flush on `AppState` background + unmount, `completed` at `ended`, signed-in only), and renders `orientation === 'vertical'` shows full-bleed under its own `VerticalChrome` with the native transport off. **Phase 2 (#97)**: `watch-segments` (POST, Bearer OR device-id: the web's `saveWatchSegments` core in `lib/watch-segments-write.ts`; anonymous flushes count only past a `trial_sessions` row for the device AND inside the positional gate — a device that watched ep 1 must not paint counters on ep 2), the feed player `mobile/src/watch/episode-feed.tsx` (ONE `FlatList` engine for both orientations; a page mounts a `<Video>` only as the current page or its neighbour — pool of 3 vertical / 2 landscape; neighbours preload paused+muted so auto-advance starts on a warm player with no black frame; landscape arms the next page 45s before the end, vertical = swipe; a locked episode's page IS its answer — the sign-up wall for `signup_required`, «Subscribers only» (no retry) for `subscribe_required` (#288); token refresh at expiry−60s with 1/2/4s backoff), the bucket tracker `use-segment-tracker.ts` (web semantics: once per continuous pass, seek re-arms, 20s flush) behind the in-memory `segment-queue.ts` (network/5xx retry with backoff + on foreground, 4xx dropped — not persisted across an app kill, registry), and PiP / background audio / now-playing via `react-native-video` props on the current page + the plugin options in `mobile/app.json` (native config → `expo prebuild --clean`). No PostHog and no Chromecast in the app (registry). Server half in `lib/api/v1.ts` (`import "server-only"`), wire types in `lib/api/types.ts` (**universal — imported by the app**, so a DTO change fails compilation instead of failing on a user's phone). Three deliberate divergences from the web, all forced by there being no browser: the token route is **POST** (side effects, never cached); the anonymous identity is a **device UUID header** (`x-matio-device-id`, validated as a canonical UUID) taking the `sessionToken` slot in `trial_sessions`, because `proxy.ts` skips `/api` for cookie minting; and the signup gate is **positional and softer than the web's** — `resolveSignupGate()` returns `{mode:'after_episodes', episodes: APP_SIGNUP_GATE_EPISODES ?? 1}` under free-mode + `REQUIRE_SIGNUP`, so the first episode of each show plays anonymously (a login wall on first launch is the likeliest App Store 5.1.1(v) rejection). `/v1/config` REPORTS that gate and `/v1/playback-token` ENFORCES it through the same function — never let those drift (the free-pivot lesson). Playback ids never cross the boundary: `shows/[slug]` returns signed **thumbnail** URLs (audience `t`, cannot mint video). Media URLs are absolutized (`absoluteMediaUrl`) — a native client has no origin to resolve `/shows/x.png` against. Free-tier tracking is best-effort: a rate limit or DB failure degrades measurement, never playback. `mobile/` is **outside the pnpm workspace** (own npm; `metro.config.js` watches `../lib` and sets `disableHierarchicalLookup` so Metro can't bundle a second React) and is excluded from `pnpm lint` / tsconfig; its tests run in the `mobile` vitest project (§Tests and coverage). **Native shell (#245, 2026-09-19)**: `src/app/(tabs)/` (Home · Browse · Account · Settings) is ONE screen of the root `Stack`; `show/[slug]`, `watch/[episodeId]` and `sign-in` are its siblings, pushed OVER the tabs — so the show page and the player never carry the bar (by construction, never a per-screen `tabBarStyle`). The bar is a JS `Tabs` (`expo-router/js-tabs`) with a custom `tabBar` — `components/glass-tab-bar.tsx`, board B1 «Clear»: floating pill inset 18 / bottom 26 + safe-area / height 64, four icons, the active tab a gold pill with its label; every tab screen pads its content by `useTabBarClearance()`. Glass = `components/glass.tsx`: `expo-glass-effect`'s `GlassView` on iOS 26+ (`isLiquidGlassAvailable()` read once at load), a translucent espresso + cream hairline everywhere else — **no blur, no `expo-blur`** (owner, 19.09; zero new dependencies). Icons = SF Symbols via `expo-symbols` on iOS, white SVGs in `assets/icons/` via `expo-image` `tintColor` elsewhere — **no `@expo/vector-icons`**. Browse's genre chips + title search are the pure `lib/catalog-filters.ts` (client-side normalisation of the admin's free-text `genre[]` — the wire contract keeps the raw strings; vitest-tested, re-exported through `src/shared/`). Home's Play and the show page's «Play · Ep. 1» share `watch/first-episode.ts` (`episodeRoute`: `signup_required` ⇒ `/sign-in` carrying the episode, which a successful sign-in REPLACES with the player; anything else ⇒ the player — #288). **Home v2 (#248, 2026-09-19, board 3 variant «c» Feed)**: under the untouched carousel + caption sits ONE vertical feed of full-width hero cards (`components/home-feed.tsx` — 16:10 still + `Scrim`, a pill, title, `MetaRow`, two lines of synopsis or the continue tile's progress bar, a 48pt `GlassSurface` Play disc; the card opens the show page, the disc plays: episode 1 via `usePlayFirstEpisode` on a show card, the player at `resume=<positionSeconds>` on a resume card). Signed in: «Up next» (one card per `/v1/continue` entry, server order) → Just released → the Popular now rail → the rest → the tagline; anonymous: the same without Up next and with NO sign-up card (owner). Membership and order are the pure `lib/home-feed.ts:buildHomeFeed` (vitest; re-exported via `src/shared/home-feed.ts`): no show twice — resume slugs and the featured show (it opens the carousel) never come back as cards; «new» beats «vertical» on a badge; the rail carries EVERY popularNow show and sits after the SECOND show card (fewer ⇒ after whatever cards there are — the rail never opens the feed); an empty catalog is an empty feed. The screen is a `FlatList` whose `ListHeaderComponent` is the carousel block as an ELEMENT — an inline component type remounts the carousel every render. The old Continue-watching rail left Home (`ContinueCard` deleted in #288); no `/v1` change. Account always says «Member» — `/v1` carries no subscription state (registry). The autoplay toggle (`prefs/autoplay.ts`, SecureStore `matio_autoplay_next`, read synchronously at a landscape show's `ended`) and the glass «‹» are the only player changes. **Orientation (#252, 2026-09-19)**: portrait everywhere, landscape in the horizontal player — `expo-screen-orientation` (the one dependency approved for it, 19.09; no config plugin) with `app.json` `orientation: "default"` (the OS allows every orientation, the locks decide per screen); the policy is `mobile/src/orientation.ts` — the root layout locks `PORTRAIT_UP` once at launch, the watch screen locks `LANDSCAPE` / `PORTRAIT_UP` by the LOADED show's orientation while it is FOCUSED (`useFocusEffect`), releases to portrait on blur (sign-in pushed from the wall is a portrait screen) and on unmount, and mounts the feed only while focused AND the window HAS that shape (`useOrientationSettled`, 1s grace per focus where the lock does not act — a `FlatList` mounted in portrait and rotated under it, or resized under a pushed screen, drops and re-creates its page: a second token, a restarted player; so the feed is unmounted on blur and comes back on the page it left, `onCurrentChange`) — the landscape page hides the status bar only while focused and places the «‹» + title by `insets.left`, the feed re-pins its page on a height change as the safety net, and iPad is untouched (a multitasking iPad ignores the lock; `ios.requireFullScreen` is an owner decision). Plan and phasing: [docs/mobile-app-plan.md](./docs/mobile-app-plan.md); phases 1–2 shipped (#96, #97), the native shell in #245, remaining work is #98 (push) and #99 (store artefacts). **Builds go through EAS** (project `@matvei-dev/matio`, Apple Team `MTFRZQ8SRX` — the owner's Individual membership shared with another app; TestFlight via the `production` profile with `--auto-submit`): runbook [docs/runbooks/mobile-builds.md](./docs/runbooks/mobile-builds.md); the first build's Apple login + 2FA is the owner's, never an agent's. The app's Bearer tokens carry no `azp` claim (native mint, no browser Origin), and the installed `@clerk/backend` rejects such tokens against any `authorizedParties` list — so on `/api/v1/*` `proxy.ts` withholds the list per request for an azp-less header token, read exactly the way Clerk reads the header (`authorizedPartiesForRequest`, #100); do not "simplify" that back to a static options object, and do not widen it past `/api/v1`. **Clerk that will not load (#253)**: `useOptionalAuth()` raises `stalled` on Clerk `status === 'error'` or after 8 s of waiting — a screen behind Clerk never shows an endless spinner (the Account tab and `/sign-in` render the honest «Sign-in is unavailable right now» + Retry); `retry` rests on the internal `IsomorphicClerk.loadHeadlessClerk` behind a feature-detect (registry) — a `<ClerkProvider key>` remount is handed the same stuck instance.

- **Free pivot (2026-07-04): payments are OFF by default — the whole site is free.** `lib/free-mode.ts:paymentsEnabled()` reads `PAYMENTS_ENABLED`; **unset (the current state) = free mode**, `PAYMENTS_ENABLED=1` + redeploy = paid mode back with zero code changes. Every rule below about trials / tiers / paywalls / checkout describes **paid mode** and is dormant while the flag is off. What free mode changes: `/api/playback-token` coerces the effective tier (signed-in → the `member` path, anonymous → the `free` path with its best-effort `kind='episodes'` tracking mint — anonymous funnel analytics + resume keep working) so it never 403s/429s; the watch page forces the gated branch (`gated = !paymentsOn || …`), presents every episode as `tier:'free'` to the player (the player locks episodes CLIENT-SIDE from that prop — coercing only the token route is not enough), and threads `freeMode` into `Player` so the series-end `ended` handler (also purely client-side) shows the neutral `seriesEnd` overlay instead of the member Paywall / free SignupWall; `saveWatchProgress` skips the subscriber-tier ownership gate and `saveTrialPosition` takes the full-tracking free branch (whose UPDATE is now filtered to `kind='episodes'` rows — a legacy `kind='preview'` row occupying the (cookie, show) slot must never receive uncapped positions or a `lastEpisodeId`); `/subscribe` + `/checkout` `redirect("/")` at the top of the page AND all three checkout server actions guard-return `{kind:'redirect',to:'/'}` as their first statement (they're independently POST-invocable); `proxy.ts` skips the Clerk sign-up bounce for `/subscribe(.*)` (falls through to UTM cookie capture; the page redirect does the bounce); the header/footer Subscribe links hide via a `paymentsEnabled` prop from `app/layout.tsx`; indexed copy (layout/twitter descriptions, OG tagline, manifest, about page, show `synopsisFallback`) and the show-page JSON-LD `isAccessibleForFree` switch to free variants; `/terms` renders a payments-disabled notice above the (untouched, counsel-pending) legal sections so the indexed page doesn't sell "$25 today" as a current fact. What free mode does NOT touch: the Stripe/Clerk/Mux webhooks, `mirrorSubscription` + `claimGuestCheckout` + `/welcome` (in-flight checkouts complete for ~24h after the flip and must still mirror/claim), `ACCESS_GRANTING_STATUSES` + `hasActiveSubscription()` (must always mean REAL subscription state — the guest duplicate guard depends on it), the billing portal (legacy subscribers cancel there; footer "Manage subscription" + user-menu item stay visible), the admin panel (tier selects keep working, just unenforced), and all dictionary keys (paid-mode copy stays for re-enable). **Re-enable checklist**: set `PAYMENTS_ENABLED=1` in Vercel (all environments) BEFORE the redeploy; verify `STRIPE_PRICE_MONTHLY` still exists (the `next.config.ts` build guard is now scoped to `PAYMENTS_ENABLED==="1"`); know that `kind='episodes'` trial rows minted during free mode read as *expired* trials on legacy all-subscriber shows (`findTrialSession` is kind-blind → instant 403 instead of a fresh 60s preview for cookies that watched during free mode — clear those rows or retier if it matters); annotate PostHog (grain era #5: `trial_play_started`/`paywall_shown`/`checkout_started` flatline during free mode, `free_episode_started` covers all anonymous playback). The ChatGPT Ads conversion switches with the mode by itself (registration → `registration_completed`, purchase → `subscription_created` on the checkout return) — nothing to move. **#164 blockers are done (2026-09-07), except `customers.del`**: erasure via Clerk `user.deleted` sets a live Stripe subscription to `cancel_at_period_end` (best-effort — a Stripe failure is logged + sent to Sentry by id, the erasure still completes) and tombstones in `erased_customers` BEFORE the `users` row goes **every** Stripe customer the account can be reached through — `users.stripe_customer_id` plus every customer `customers.search` finds by the account's address (#223: a guest checkout mints a Customer from the address typed on the form, a later signed-in purchase overwrites the id in `users`, and only Stripe remembers the first; one page ≤100, the cancel's 5s ceiling, no retries, best-effort — a failed search is logged + sent to Sentry by id and left to the hand path in runbook §4, because the address leaves with the row and no script re-run can repeat it). Two guards on that search: it runs only for an account with a Stripe footprint — a customer id on the row or any `subscriptions` row — so a free/gate-era address never reaches Stripe for the first time at erasure (`skipped_no_stripe_footprint`), and only customers whose address IS the account's are tombstoned — Stripe's `:` on a string field matches superstrings (`a@b.com` also returns `a@b.com.mx`), and a wrong tombstone would strand a paying stranger without an account — so a guest-checkout customer's later webhook (`metadata.guest="1"` never expires) can no longer resurrect the account through `claimGuestCheckout` — `mirrorSubscription` and `/welcome` ask `isErasedCustomer` first. Migration 0025 (`erased_customers`, expand-only) is a prerequisite of the RELEASE that carries the handler, not of the flip: the webhook reads the table in free mode too, so staging → prod before that release. The Stripe Customer itself (email, billing address) is NOT deleted — whether to `customers.del` is the owner's decision, tracked in `docs/registry.md`.
- **Signup gate (2026-07-16): `REQUIRE_SIGNUP=1` = no playback without an account — free-mode only.** `lib/free-mode.ts:signupRequired()` is true only when payments are OFF **and** `REQUIRE_SIGNUP === "1"`; in paid mode the flag is a deliberate no-op (the per-episode tier system owns gating there — no second flag matrix). **Since #198 (2026-09-08) the gate is PER-EPISODE, not blanket**: the admin's `episodes.access` decides for anonymous visitors through the one pure rule `resolveEffectiveTier` (`lib/episode-access.ts`) — `free` plays with no account, `member` walls, and `subscriber` **also walls as member while payments are off** (a paywall CTA would lead to `/subscribe`, which redirects home in free mode; flip `PAYMENTS_ENABLED=1` and the paid branch turns it into a real paywall with no code change). Every consumer reads that one rule so the seams cannot drift (the free-pivot lesson): the watch page's `tier` prop (mode `free` + a locked tier ⇒ `isEpisodeLocked` ⇒ `SignupWall` full-surface, **prop-driven with zero token fetches/mints** — the same path a paid-mode member-tier deep link takes), `/api/playback-token` via `resolveRequestTier` (which also knows about the session: signed-in + payments off ⇒ member path) → 403 `reason:'signup_required'` for a walled episode, `saveTrialPosition`, and `saveWatchSegments` (anonymous flushes carry `freeTierOnly`, enforced inside `saveWatchSegmentsFor` on the episode row it already reads). `autoplay` is now switched off only when the STARTING episode is locked, and the wall keeps its copy variant ("Watch for free" / "Mira gratis", dict keys `signupWall.gateKicker`/`gateBody`; PostHog `signup_wall_shown` gains a `gate` property). Show-page JSON-LD `isAccessibleForFree` is computed per episode from `isFreeToWatch` — true only where an anonymous visitor really gets the video (Google treats registration walls like paywalls — claiming "free" for account-gated video reads as cloaking). Signed-in users are untouched: member path plays everything, series-end keeps the neutral `seriesEnd` overlay via `freeMode`. After Clerk sign-up the wall redirects back to `/watch/<slug>?ep=<target>`, where the member branch already does attribution stamping + `CompleteRegistrationPixel` + trial-row linking. The hero teaser on `/` deliberately stays public (inline-minted JWT, not the token route). Analytics: anonymous playback stops ⇒ no new `trial_sessions` rows ⇒ the free/organic dashboard funnel (Sessions/Played/2+/3+, sources rollup, tracked-link stats, show depth) flatlines from the flip (**grain era #6**); the funnel becomes signups (`signup_origin='clerk_signup'`) + `watch_progress` engagement + PostHog `signup_wall_shown(gate:true)` → `signup_completed`. **Dashboard re-point (2026-07-18, SUPERSEDED same day)**: a PostHog-backed "Signup funnel" panel briefly re-pointed the free-mode dashboard; the full v2 redesign (see "Analytics dashboard v2" rule) replaced the whole free-mode page hours later, retiring that panel — `loadSignupFunnelDb` survives only inside the dormant legacy dashboard; `lib/posthog-query.ts` itself is still the live HogQL transport behind `/admin/analytics/sessions` (the `POSTHOG_PERSONAL_API_KEY` env pair stays required for THAT page; `/admin/analytics` itself is first-party).
- **Analytics dashboard v2 + first-party audience measurement (2026-07-18)**: `/admin/analytics` in free mode is the spec'd retention dashboard ("does the content hook people enough that they come back?") — 7 blocks: KPI row (Visits / Registrations+conv / North-Star deep-watch ≥80% of first episode / release retention), pulse chart (rolling-WAU line over daily new/returning/lost bars + release markers; "lost" = 7 straight days without a watch), cohort funnel (site → show → wall → registered → started → 25/50/80/100%), geo (tile-grid world map + country table), source×geo quality matrix, content block (episode table + clickable YouTube-Studio-style retention curve + watch-time widget), per-show release retention (% of ep-N finishers starting ep N+1 within 7 days of release). Data model: **consent-EXEMPT first-party measurement** — `matio_aid` httpOnly cookie (13 months, minted once in `proxy.ts`, never refreshed, skips bots/non-GET/api/admin; documented on /cookies + /privacy as strictly-necessary audience measurement, legal basis legitimate interests) + `/api/t` sendBeacon endpoint writing `visitors` (write-once first-visit UTM/referrer/country; raw IP never stored) and `visitor_days` (aid×day with home/show/wall flags); the wall flag is also stamped server-side by `markSignupWallShown` and the token route's 403. `linkVisitorToUser` (signed-in watch render, next to `applyUserAttribution`) merges visitor→user ("склейка", first link wins, funnel counts only accounts YOUNGER than the visitor row) and stamps `users.country`. Watch side: `saveWatchProgress` also upserts `watch_days` (user×day) and maintains monotonic `watch_progress.max_position_seconds` (`completed` keeps live flip-on-rewatch semantics — the continue-watching rail depends on it; analytics derive "finished" as `completed OR max_position ≥ 95%·duration`); the player marks 10s buckets on timeupdate (dedupe per continuous pass, seek re-arms — rewatch peaks are intentional) and flushes ≤120 buckets/20s to `saveWatchSegments` → `watch_segments` (episode×day×bucket counters) + `total_watched_seconds`. `episodes.released_at` is stamped write-if-null by the show-publish action AND the Mux ready webhook (published shows only); back-catalog release dates fall back to first observed watch in queries. Data layer `lib/admin-analytics-v2.ts` (fraction params need `::float8` casts — pg types bare numeric params as int against int columns); the single raw-SQL exception is the `generate_series` rolling-WAU query (`gs.d` must be cast `::date`). Page = Suspense islands per block; charts are static server SVG (`components/admin/spec-charts.tsx`; palette validated against `#0f0a07`); admin dict section `analyticsSpec` (ru/en). **Caveats**: ledgers accrue from deploy (older ranges read zero — the migration backfills `first_watched_at`≈`updated_at`, `max_position`≈`position` only); since 2026-09-04 the v2 page is the dashboard in BOTH modes — the legacy page (MRR / subscription mix / paid funnels) is reachable in paid mode only, via `?view=legacy` (`lib/admin-analytics-view.ts`); `/admin/links` still generates tracked links but the legacy link-stats panel lives on that legacy page. 
...
- **Subscriptions**: single monthly plan at **$25/mo, charged at checkout** (#207, 2026-09-09) — no annual, no other tiers, **no trial and no intro fee**. Checkout sends ONE line item (`STRIPE_PRICE_MONTHLY`) built by `lib/checkout-session-params.ts:buildCheckoutSessionParams` — the same builder for the signed-in and guest flows, so what is sold and on what terms cannot drift between them; its test pins "one line item, nothing named trial". The sub is `status='active'` from day 0 and renews monthly. **Conversion value = the price actually charged**: Meta `Purchase` + PostHog `subscribe_succeeded` fire once, on the transition into an access-granting status, valued at the subscription item's `unit_amount` (`MEMBERSHIP_VALUE = 25` for `InitiateCheckout`/`checkout_started`). The previous $1 / 3-day intro trial (2026-06-11 → 2026-09-09) is gone: `lib/checkout-trial.ts`, `STRIPE_PRICE_TRIAL_FEE` and the day-0-$1 reporting rule went with it, and the `next.config.ts` build guard now requires only `STRIPE_PRICE_MONTHLY`. Nobody was grandfathered because nobody was on the old price: at the switch the table held zero access-granting rows (a Stripe price change never re-prices existing subscriptions anyway — they keep the price they were created with). The guest checkout action is IP/hour rate-limited in-app (see Production context). Stored in `subscriptions` table; mirrored from Stripe via webhook (one source of truth). The `subscription_plan` enum still carries an `'annual'` value for any historical (pre-launch test) rows but no new row is ever written with it. The webhook is idempotent — every delivery claims its `event.id` in `stripe_events` before processing; duplicates short-circuit. Partial unique index on `(user_id) WHERE status IN ('active','trialing','past_due')` guarantees at most one access-granting row per user; historic rows (status='canceled') stay. Every subscription gate also checks `current_period_end > now()` so a dropped `customer.subscription.deleted` webhook can't extend playback past the user's term.
...
- **In-site checkout (Embedded, 2026-06-14)**: both checkout entry points — the signed-in `/subscribe` CTA and the signed-out pay-first paywall CTA — are `<Link>`s to **`/checkout`** (`app/checkout/`), which renders **Stripe Embedded Checkout** (`@stripe/react-stripe-js` `<EmbeddedCheckout>`) in an iframe **on our domain** — the buyer never leaves `matio.tv`. `app/checkout/checkout-client.tsx` calls the unified `createCheckoutSession` server action (`app/checkout/actions.ts`) on mount; it resolves auth **server-side** and dispatches to `createAuthCheckoutSession` (signed-in) or `createGuestCheckoutSession` (guest) — the **same** session builders as before, with **all** prior config intact (the single $25/mo line item, `automatic_tax`, billing-address collection, ToS-withdrawal `consent_collection`, locale, and the full attribution/CAPI/guest-claim metadata). The only Stripe change is `ui_mode: "embedded_page"` (the pinned API version — `STRIPE_API_VERSION` in `lib/stripe.ts`, held to the installed SDK by `lib/stripe-api-version.test.ts` — names the value `embedded_page`, not the older `embedded`) + `return_url` instead of `success_url`/`cancel_url`; the action returns a `CheckoutSessionResult` (`lib/checkout-session.ts`) the client handles — `embedded` (mount the iframe), `hosted` (full-navigate to Stripe), or `redirect` (guard bounce via `router.replace`). After payment, `redirect_on_completion` defaults to `always` so Stripe sends the **top frame** to `return_url` (`/welcome` for guests, the watch path for signed-in) — the `/welcome` claim + webhook-mirror machinery is **untouched**. **Graceful degradation**: when `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` is unset (server `embeddedCheckoutEnabled()` false), the actions create a **hosted** session and `/checkout` full-navigates to `checkout.stripe.com` — the exact pre-embedded behavior, so a deploy missing the key never breaks checkout. NEXT_PUBLIC is build-time-inlined, so set the key in Vercel **before** the deploy that wants embedded. The checkout-intent events (Meta `InitiateCheckout` + PostHog `checkout_started`) still fire server-side inside the builders (now at `/checkout` mount, not CTA click); `signup_cta_clicked` still fires at the CTAs. **Webview caveat**: Embedded Checkout is an iframe to `js.stripe.com` — in restrictive in-app browsers (Instagram/Facebook), iframes + Apple/Google Pay can be flaky vs. a full hosted page; `OpenInBrowserHint` is kept on the pay-first paywall to nudge those users out to a real browser. Stripe Dashboard → Branding controls the embedded form's look (no client-side `appearance` API for Embedded Checkout). **One open Checkout Session per buyer (#217, 2026-09-15)**: the signed-in builders (`createAuthCheckoutSession` AND `createAuthWalletCheckoutSession`, both through `createSoleOpenSession` in `app/subscribe/actions.ts`) create the session **without an idempotency key** and then `checkout.sessions.list({customer, status:'open'})` + `expire` every other open session of that customer — create-then-sweep, so two same-instant creates cannot both see an empty list (whichever lists last expires the other); a sweep that cannot be completed expires the new session and throws (fail closed — a client secret is handed out only once the invariant is proven). The hour-bucketed `checkout:<userId>:<hour>:<digest>` key is GONE on the auth flow, on purpose: its digest hashed request snapshots (consent cookie, `capi_ip`, `attr_last_*`, the `resume` playhead, the wallet/embedded surface), so two tabs routinely got two keys and two live billable sessions — and next to the sweep a key is worse than none, because Stripe replays the CACHED first response for a key, `status:'open'` included, so a retry would get a dead session back for the rest of the hour with no way to tell. `customers.create` DOES carry `customer:<userId>` (two parallel first checkouts used to mint two customers; the users write-back was last-writer-wins and one session sat on a customer the row no longer named — paid, no access). Clients survive an expired session: the wallet's confirm error hides the slot and shows `checkout.walletFailed` (no re-arm — that would expire the checkout the buyer is paying in elsewhere); `/checkout` keeps the session id (`CheckoutSessionResult.embedded.sessionId`) and on every `visibilitychange`→visible AND every `window` `focus` (two same-instant creates can expire each other — the visible tab never gets a visibilitychange) asks `checkoutSessionState(sessionId)` (`app/checkout/actions.ts`, read-only, uncertainty answers `open`, and it answers only about the CALLER's session — signed-in: `session.customer` = the user's `stripeCustomerId`; guest: `client_reference_id` = the `checkout_claim` cookie; no binding → `open` before Stripe is asked, so an anonymous caller cannot make the server look up arbitrary `cs_…` ids) — `closed` swaps the dead iframe for the retry card with `checkout.expiredBody`; the retry is a reload, i.e. a fresh session, which in turn closes the newer one elsewhere (the tab the buyer is looking at is the live one). The sweep pages by re-listing the first page until `has_more` is false (10 rounds max, then fail closed) — a `starting_after` cursor would point at an object the loop itself just expired out of the filtered list. Removing the key also removed the only brake on session creation for a signed-in buyer (reloads / re-ticks each mint a session + `checkout_started`) — **closed by #227 (2026-09-15)**: `prepareAuthCheckout` runs the guest flow's hourly counter (`lib/checkout-rate-limit.ts:checkoutRateLimited`, the same `guest_checkout_attempts` table, key `user:` + HMAC(userId) with the trial salt — no Clerk id in the table) right after the auth guard and BEFORE `customers.create` / `createSoleOpenSession`, counted on every call, `AUTH_CHECKOUT_RATELIMIT_PER_HOUR` = 10 by default; over it `createAuthCheckoutSession` throws `CheckoutRateLimitedError` (`lib/checkout-session.ts`), which the `createCheckoutSession` dispatcher turns into a `{kind:'rate_limited'}` result (#233 — a code, because a thrown message is masked in prod; only that class, every other throw still rejects), and `/checkout`'s client shows `checkout.rateLimitedBody` ("try again in an hour") on the same card with NO retry button (a retry inside the hour only burns an attempt; every other failure keeps `errorBody` + `retry`) and the wallet answers `unavailable` (the slot simply does not render). Fail-open on a DB error like the guest brake, logged as `startCheckout: rate limited { userId }` — id only (log audit). `resume` stays in `return_url` and the `surface` param stays on `buildCheckoutSessionParams` (it picks the consent pair, not a key). Residual, accepted: a buyer who pays in tab A and, within the ~1s before Stripe lists the new subscription, creates AND pays in tab B — the mirror's partial unique index refuses the second row (manual refund). The guest flow, which has no customer to list by, got its own half of the invariant in #224 (see "Pay-first guest checkout"). Rationale + rejected designs: [ADR 0002](./docs/adr/0002-single-open-checkout-session.md).
- **In-place wallet button on the paywall (`WALLET_EXPRESS_CHECKOUT=1`, #210)**: Apple Pay / Google Pay rendered INSIDE the paywall overlay so a viewer who hits the wall pays without leaving the player. Surface is Stripe's `ExpressCheckoutElement` from **`@stripe/react-stripe-js/checkout`** driven by a **`ui_mode: 'elements'`** Checkout Session — note the enum: on the pinned API version (`STRIPE_API_VERSION`, `lib/stripe.ts`) the installed SDK names exactly `hosted_page` / `embedded_page` / `form` / `elements`; the old `hosted`/`embedded`/`custom` hard-error — at the API, not in `tsc`: the SDK's `UiMode` type ends in an open `| OtherString`. Deliberately the SAME Checkout Session object `/checkout` builds, from the SAME `buildCheckoutSessionParams` (now taking a `surface` discriminator), so price, `automatic_tax`, `billing_address_collection`, `customer_update`, `locale` and the whole `subscription_data.metadata` channel are byte-identical — `planFromPriceId` still resolves, Stripe Tax still persists on the subscription, and the webhook mirror / attribution / CAPI / guest machinery is **untouched**. `lib/checkout-session-params.test.ts` pins that the two surfaces differ in NOTHING but the consent pair. **The consent pair is the one thing that moves**: `custom_text` is hard-rejected under `ui_mode:'elements'` (verified against the live API — `400 … not supported with ui_mode: elements`) and `consent_collection`'s only renderer, `TermsElement`, is beta-locked in the installed SDK with zero options — so **one required checkbox of our own** collects BOTH the Terms acceptance (link to /terms) AND the EU/UK 14-day withdrawal waiver, in the buyer's language (`subscribe.walletConsent`; /privacy is a notice link beneath it, never part of the "I agree"), and `subscription_data.metadata` records only `tos_version` (`lib/wallet-checkout.ts`). **No acceptance time in the record, on purpose (#214):** the exact moment is the session's own `created`, and a second clock in the record would be a second, contestable answer to the same question (the original second reason — a clock read rode the create params into the hour-bucketed idempotency digest — went with that key in #217). The tick is client-asserted — the server refuses a client that says "unticked" and cannot prove a human ticked. No new column and no new store on purpose (data minimisation — `/gdpr` data map); unlike the `capi_*` keys `tos_version` is **never scrubbed**. The checkbox necessarily precedes the session (an Element cannot mount without a client secret), which is also why this does NOT mint a Stripe session per wall impression — ticking the box is the intent signal. `InitiateCheckout` + `checkout_started` therefore fire at **confirm** time (`reportWalletCheckoutStarted`), keyed on the session id so Meta dedups, keeping the saved PostHog funnel's meaning. One pure rule decides whether the surface may exist at all — `resolveWalletGate` (`lib/wallet-checkout.ts`): payments off → redirect; flag off / no publishable key / **in-app browser** / anonymous / consent unticked → `unavailable`, which is a normal state where the paywall simply keeps its card CTA (Apple's Acceptable Use Guidelines want the wallet ALONGSIDE other methods, never replacing them). **v1 is signed-in only** — a guest wallet purchase also mints a Clerk account and a sign-in ticket from the wallet's email, i.e. the path the 2026-06-16 webview incident hardened; separate PR, registry row. Elements (unlike hosted Checkout) **requires payment-method-domain registration per hostname, separately in sandbox and live** — `matio.tv` is registered and active, `matio-staging.vercel.app` is not; there is no `/.well-known/` file step any more (Stripe dropped it in early 2025 — proof: matio.tv is active while that URL 404s), though `lib/staging-lock.ts` opens the whole `/.well-known/` prefix defensively since Apple re-verifies on the TLS-certificate lifecycle. Known limits, all in `docs/registry.md`: the owner has not yet picked among the five Lab variants (`lab/wallet-slot-variants.tsx`, ships A), Google Pay may be suppressed outright by `automatic_tax` without `shipping_address_collection` and is unproven on real Android, and on iPhone **fullscreen has no DOM overlay at all**, so the wall — and the button — only exist inline.
- **Pay-first guest checkout (`PAY_FIRST_CHECKOUT=1`, "invisible account", 2026-06-09)**: with the flag on, the paywall's **signed-out** CTA links to `/checkout`, which calls `createGuestCheckoutSession` (`app/subscribe/guest-actions.ts`) — a Checkout session with NO Clerk step (no `customer`/`customer_update`; Stripe creates the Customer from the email typed on the checkout form). The purchasing browser is bound to its session via the httpOnly `checkout_claim` cookie mirrored into `client_reference_id` (cookie reused across tabs, so every tab of one browser is ONE buying party). **One open session per guest buyer (#224, 2026-09-15)**: the create carries **no idempotency key** (the old `checkout:guest:<token>:<hour>:<digest>` hashed drifting snapshots — `resume`, `capi_ip`, `attr_last_*`, the consent cookie — so two tabs got two keys and two live billable sessions, and Stripe replays a key's CACHED `status:'open'` body long after a sweep killed the session); instead, right after the create, `claimSoleGuestSession` (`lib/guest-checkout-sessions.ts`) records the new id in `guest_checkout_sessions` under an **HMAC of the claim cookie** (never the value — the cookie is the /welcome sign-in-ticket binding) with ONE statement, `INSERT … ON CONFLICT DO UPDATE … RETURNING old.session_id` (Postgres 18: `old` is the conflicting row's pre-update value AFTER waiting on its lock, NULL on the insert path — verified on a local PG18 with two sessions; a `(SELECT …)` subquery in RETURNING reads the statement's START snapshot and answers NULL to BOTH parallel claims, which is the bug), and the previous id it hands back is expired by id through `expireIfOpen` (`lib/checkout-session.ts`, shared with the signed-in sweep: an `expire` refused because someone else closed it first is fine; still `open` after a failed expire propagates). Fail closed like `createSoleOpenSession` — a DB or Stripe failure in that step expires the new session and throws (an unrecorded session is one the NEXT creation can never find); and because the claim precedes the expire, a failed expire of a still-open previous session **rolls the claim back** (`rollBackGuestClaim` — `previous` written back with the same upsert, so the next checkout retries that same session; a rollback that displaces a third tab's id, or is itself refused by the DB, leaves one session open and unrecorded until Stripe expires it — logged by ids, ADR 0002 residual + registry). The per-IP brake stays `guestCheckoutRateLimited` (fail-open, anti-abuse). Rows self-prune after 24h (a Checkout Session's own maximum life) on 5% of claims — a technical key, not a `/privacy` window, so not in `lib/retention.ts`; data-map row. The `/checkout` refocus probe already binds a guest by `client_reference_id` = the cookie, so the second tab learns its session is dead the moment it comes back. `subscription_data.metadata` carries `guest=1` + `claim_token` + `trial_token` (the trial cookie, for exact-token trial→paid linkage) plus the usual attr_*/capi_*/ph_consent keys. **Claim** (`lib/guest-checkout.ts:claimGuestCheckout`, idempotent, THROWS on failure): resolve email from the Stripe customer → find-or-create the Clerk user (`createUser({skipPasswordRequirement:true})` — passwordless, email-code is the canonical credential) → upsert the users mirror with `stripeCustomerId` → link trials by exact token + stamp attribution from metadata. Runs from BOTH the webhook (`lib/subscription-mirror.ts` no-user branch — a throw rolls back the `stripe_events` claim so Stripe retries; never warn-and-return a guest sub) AND the `/welcome` success page (closes the webhook-vs-redirect race; both writers idempotent, whichever lands first wins). **The one exception to "never warn-and-return a guest sub" is an ERASED customer** (#164): both callers ask `isErasedCustomer(customerId)` (the `erased_customers` tombstone written by Clerk `user.deleted`) BEFORE the claim — the webhook consumes the event with a warn by id, `/welcome` redirects home — because `guest="1"` never leaves the metadata and the claim would otherwise re-create the Clerk user and the `users` row with the address. That check is data about one customer, not a payments-mode branch, and stays in the mirror/claim path. The guest **duplicate guard** lives in `mirrorSubscription`: a guest sub whose user already holds an access-granting row is NOT mirrored — loud `console.error`, manual refund (alert-only by design; never auto-refund from a webhook). `/welcome` mints a Clerk sign-in ticket (`signInTokens.createSignInToken`, 600s) **only when the `checkout_claim` cookie equals the session's `client_reference_id`** — the no-cookie/webview path degrades to email-code sign-in with a masked email, NEVER token minting (the URL-borne `session_id` is shareable; that cookie check is load-bearing security). Ticket consumed client-side via the signal-based `signIn.ticket({ticket})` + `signIn.finalize()` (`components/welcome/ticket-sign-in.tsx`). Lead/CompleteRegistration + signup_completed fire post-sign-in on /welcome (AFTER Purchase — funnel reorder: don't optimize Meta ad sets on Lead while the flag is on). The flag also disables the expired-trial server redirect to `/subscribe` on the watch page (the player's token 403 renders the pay-first paywall instead). Flag off → everything falls back to the auth-first flow. **Clerk prod prerequisite: "Email verification code" sign-in must be enabled** (the created accounts have no password). Stripe's no-code Customer Portal login page is the account-less cancel safety net.
- **Subscribe surface**: `/subscribe` shows a single membership card — `$25` / `month` / "Charged today · cancel anytime" (EN), `25 $` / `mes` / «Se cobra hoy · cancela cuando quieras» (ES) — no plan picker. The in-player paywall (`components/watch/paywall.tsx`) leads with **sign-up**, not pricing — the in-player CTA opens Clerk's `<SignUpButton mode="modal">` with `forceRedirectUrl=/subscribe?show=…`; its pay-first CTA reads `Hazte miembro · 25 $/mes` / `Become a member · $25/mo`. Signed-in non-subscribers (paid→canceled→returned) skip the sign-up step via a direct `<Link>` to `/subscribe`. Checkout's `line_items` is the single $25/mo price; it collects billing address (`customer_update.address: "auto"` when a customer exists, `billing_address_collection: "required"`), runs `automatic_tax: { enabled: true }` on a `tax_behavior=exclusive` price (so tax stacks on top of $25 — currently $0 until a Stripe Tax registration is added), and requires the EU/UK 14-day-withdrawal waiver via `consent_collection.terms_of_service: "required"` + localized `custom_text.terms_of_service_acceptance`. `locale` is passed so the checkout matches the site language. The `/subscribe` CTA is now a `<Link>` to `/checkout` (in-site Embedded Checkout — see next rule), not a `<form action>` redirect.
...
- **Auth gating**: `proxy.ts` sends unauth'd `/subscribe(.*)` requests to Clerk's **sign-up** page (not sign-in) — most paywall conversions are first-time users; Clerk's sign-up page still links to sign-in for the minority case. Admin routes keep `redirectToSignIn` since admins already have accounts. **Authorized parties (#100)**: `clerkMiddleware` gets `authorizedParties` from `lib/authorized-parties.ts` — built at deploy from the environment (`VERCEL_PROJECT_PRODUCTION_URL` + `NEXT_PUBLIC_APP_URL`) plus the hard-pinned `SITE_URL`, applied only on `VERCEL_ENV === "production"` deployments: prod → `https://matio.tv` + `https://www.matio.tv`, staging → `https://matio-staging.vercel.app` (+ the same matio.tv pair, harmless there), **Vercel previews and localhost → `undefined`** (preview URLs are per-commit and cannot be listed; a fixed list would sign the reviewer out). Garbage in a variable is skipped; `SITE_URL` is always in the list because a valid-but-wrong value would otherwise sign all of prod out (`TokenInvalidAuthorizedParties` is not a handshake reason). The installed `@clerk/backend` (3.18.1 since #242 — the same `!azp || !authorizedParties.includes(azp)` check as 3.17.1) rejects a token with NO `azp` when a list is set, so on **`/api/v1/*` only** the per-request options callback withholds the list when the header token has no `azp` — the app's native `@clerk/expo` tokens — reading the header **exactly as Clerk's `parseAuthorizationHeader` does** (case-sensitive `Bearer`; when Clerk finds a header token it verifies that, never the cookie — a looser reading would let a cookie session skip its `azp` check). Everywhere else the list always applies; a browser-minted token (always with `azp`) replayed as a Bearer is still held to it. Never hard-code the rest of the list, never apply it on previews, never widen the exemption beyond `/api/v1`.
...
- **Branching video (2026-09-06 #143 schema + admin; 2026-09-07 #144 the
  ...
  **Branches are hidden from every public list**: the show page + its JSON-LD
  ...
  `catalog` + `shows/[slug]`, and a filter on `continue`'s tiles since #288;
  DTOs untouched). **`deleteSeason` deletes the
...
- **ChatGPT Ads pixel (OpenAI `oaiq`, 2026-09-03)**: consent-gated advertising measurement, same pattern as the Meta Pixel and GA4 — `components/site/openai-pixel.tsx` injects `oaiq.min.js` (loaded directly, **no npm SDK**) only after `cookie_consent.marketing === true`, sharing the layout's `initialConsent`. OpenAI's SDK has a first-class consent switch: on withdrawal the loader calls `oaiq('consent', false)` (halts every ping; the SDK persists the denial in its own `__oaiq_consent` cookie) and stops its own calls via `consentRef`; a re-grant calls `oaiq('consent', true)`. The SDK defaults consent to TRUE at init — acceptable only because we never init before consent. `page_viewed` is measured by the snippet on load and by the loader on App-Router route changes (the SDK fires nothing on navigation by itself). **The campaign conversion is `subscription_created`** (`type: plan_enrollment`) — the event the owner configured in OpenAI Ads Manager — and its placement follows the mode (2026-09-04): **free mode** → `components/site/complete-registration-pixel.tsx` at account materialization (`plan_id: "free-membership"`, `event_id: signup:<userId>`) because creating an account is the only plan enrollment that exists; **paid mode** → `components/site/purchase-pixel.tsx` on the checkout return — `/welcome` for pay-first guests (session verified PAID server-side, amount = `session.amount_total`) and the watch page / home for signed-in buyers, where `app/subscribe/actions.ts` appends `cs={CHECKOUT_SESSION_ID}` to `return_url` and `lib/checkout-return-verify.ts` verifies that session at Stripe exactly like /welcome does for guests — completed, PAID, and its customer = THIS user's `stripeCustomerId` (a forged or foreign id yields nothing, so the beacon cannot fabricate conversions) — then runs the idempotent `mirrorSubscription` inline (closing the redirect-before-webhook race for signed-in buyers) and renders the beacon on every watch branch (`plan_id: OAIQ_MEMBERSHIP_PLAN_ID`, `amount`/`currency` = what Stripe actually charged, `purchaseKey`/`event_id` = the **subscription id** — the same key Meta CAPI uses, so a future OpenAI Conversions-API call dedupes by construction). In paid mode a registration reports `registration_completed` instead (`paymentsEnabled` prop). Every beacon is once-per-key via localStorage and burns its flag only under LIVE consent. Only documented fields go into params — the SDK rejects unknown ones. `debug` is on only in `development`. Env: `NEXT_PUBLIC_OPENAI_PIXEL_ID` (public/build-time) — **blank → fully off**; **set on 2026-09-04** in the prod project (Production + Development; Preview deliberately skipped) and in the staging project (verified baked into the staging build) — prod picks it up at the next release build. Helpers in `lib/openai-pixel-events.ts` (`measureOaiq` / `onOaiqReady`, no-ops until loaded). Cookie policy (`/cookies`) lists `__oppref` / `__oaiq_consent` and names OpenAI Ireland Ltd.
...
- Don't add `paymentsEnabled()` branches to the webhook/mirror/claim path
  ...
  belongs at access-gate call sites and payment SURFACES only. (The
  `isErasedCustomer` check in front of `claimGuestCheckout` is not a mode
  branch — it is a fact about one customer, true in both modes — and stays.)
...
- **Business entity**: UK **limited company** — **DEEP ORDINARY LTD**, company
  ...
  (the former sole-trader identity) everywhere user-facing. Not VAT-registered
...
- **Pay-first checkout (`PAY_FIRST_CHECKOUT`): the var IS present in Vercel Production** (set 2026-06-10 under paid mode; value Encrypted — re-read it in the Vercel dashboard before re-enabling payments, don't assume it's off). **LIVE since 2026-09-09** (paid mode, next bullet): the paywall's signed-out CTA leads here, so the guest double-session gap is a live bug (#224, p1). Prerequisites (written for the re-enable, kept as the checklist they were): (1) ~~Clerk email-code sign-in~~ **DONE — enabled in prod** (verified during the 2026-06-16 webview incident); (2) activate Stripe's **no-code Customer Portal login page** (`billing.stripe.com/p/login/…`, email + OTP) as the account-less cancel safety net; (3) ~~Vercel WAF rate-limit~~ **DONE in-app (2026-06-13)** — `startGuestCheckout` is now rate-limited per (IP hash, clock-hour) via the `guest_checkout_attempts` table (`lib/checkout-rate-limit.ts`, default 30/hr, env `GUEST_CHECKOUT_RATELIMIT_PER_HOUR`), checked right after the auth guard, before any Stripe session / Meta+PostHog work; over-limit degrades into the auth flow. Done in code rather than a WAF rule because that server action shares the `POST /watch/*` endpoint with `saveWatchProgress` — a path-level rule would throttle progress saves. **The signed-in builders got the same brake on 2026-09-15 (#227)**: `checkoutRateLimited("user:" + HMAC(userId), AUTH_CHECKOUT_RATELIMIT_PER_HOUR)` in `prepareAuthCheckout`, default 10/hour per account (env-overridable, same table, same fail-open) — over it `/checkout` says "try again in an hour" with no retry button (#233) and the wallet slot does not render; no separate Vercel env var is set for it today (the default applies). Funnel note: with the flag on, Meta `Lead`/`CompleteRegistration` + PostHog `signup_completed` fire AFTER `Purchase` (on /welcome), so don't optimize ad sets on `Lead` and annotate PostHog (trial-metrics grain era #4 for the funnel reorder).
- **PAID MODE since 2026-09-09 — the free pivot (2026-07-04 → 2026-09-09) is over.** `PAYMENTS_ENABLED=1` is set in Vercel Production, together with `STRIPE_PRICE_MONTHLY=price_1UDhk3CGXbzphNyzGUObyyrc` ($25/mo, #207/#208), `PAY_FIRST_CHECKOUT` (guest checkout on), `NEXT_PUBLIC_APP_URL=https://matio.tv` (the checkout return origin — #202 came from the bench returning localhost) and `CRON_SECRET`; `WALLET_EXPRESS_CHECKOUT` is **not** set in prod (staging only, until the #217 two-tab rehearsal, #215 and the lawyer). Access is the per-episode tier system again (the "Per-episode access control" rule): `free` plays anonymously, `member` walls to sign-up, `subscriber` walls to the $25 paywall. Live layout (prod DB, 2026-09-15): `the-scarlet-oath` ep 1–2 free · ep 3 member · ep 4–6 subscriber; `quedate-conmigo` ep 1 free · ep 2 member · ep 3 subscriber; `fallen` subscriber; `second-hand` free; the single-episode shows (`cartero-mundo`, `juego-de-seduccion`, `morelli`, `protocol-1500`) member — admin-set on the season page, re-read the DB before quoting it. Staging rehearses the same thing with **test-mode Stripe** (sk_test/pk_test, its own `STRIPE_WEBHOOK_SECRET`, a sandbox payment-method domain for `matio-staging.vercel.app`, a test-mode Stripe Tax head office) and its own tier layout copied from prod — the end-to-end test purchase passed there on 2026-09-09; **a live $25 purchase on prod has not been verified yet** (owner item). What the switch did NOT change: zero access-granting `subscriptions` rows existed at the flip (7 historical), so nobody was re-priced; the Stripe env vars must stay (build guard + mirror); the "Free pivot" rule above still documents free mode exactly, for the day the var is unset again (runtime read: unset + redeploy = free). PostHog grain era #7 starts 2026-09-09: `trial_play_started` / `paywall_shown` / `checkout_started` / `subscribe_succeeded` accrue again, `free_episode_started` covers only the `free`-tier episodes.
- **Signup gate — DORMANT since 2026-09-09 (was live 2026-07-16 → 2026-09-09)**: `REQUIRE_SIGNUP=1` is still set
  in Vercel Production, but `signupRequired()` is false by construction in paid
  mode (the tier system owns gating there); the var stays for the day payments
  are switched off again. The history below describes the free-mode era. One-command
  check (free mode only): `curl -s https://matio.tv/api/v1/config | jq .signupGate` →
