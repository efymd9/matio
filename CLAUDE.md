...
- **Mutations are idempotent.** A client retry — double tap, lost response,
  ...
  (`customers.create` → `customer:<userId>`; the GUEST checkout session's
  `checkout:guest:<token>:<hour>:<digest>`), **an expire-sweep where they
  can** — the signed-in checkout creates its session with NO key and then
  expires every other open session of the customer (`createSoleOpenSession`,
  #217; a replayed key hands back the cached body of a session the sweep has
  since killed) — and a claim-stamp under `FOR UPDATE SKIP LOCKED` for batch
