...
describe("PurchasePixel — ChatGPT Ads subscription_created on checkout return", () => {
  it("measures the real charge once per Checkout Session, keyed for server dedupe", () => {
    const oaiq = vi.fn();
    window.oaiq = oaiq as unknown as NonNullable<typeof window.oaiq>;
    const view = render(
      <PurchasePixel checkoutSessionId="cs_test_abc" amountCents={100} currency="usd" />,
    );
  ...
  it("waits for the SDK and never burns the flag while it is absent", () => {
    render(
      <PurchasePixel checkoutSessionId="cs_test_wait" amountCents={100} currency="usd" />,
    );
    expect(localStorage.getItem("matio:oaiq:purchase:cs_test_wait")).toBeNull();
    const oaiq = vi.fn();
    window.oaiq = oaiq as unknown as NonNullable<typeof window.oaiq>;
    window.dispatchEvent(new Event(OAIQ_READY_EVENT));
    expect(oaiq).toHaveBeenCalledTimes(1);
    expect(localStorage.getItem("matio:oaiq:purchase:cs_test_wait")).toBe("1");
  });
  ...
  it("does not measure — or burn the flag — without live marketing consent", () => {
    ...
    window.oaiq = oaiq as unknown as NonNullable<typeof window.oaiq>;
    render(
      <PurchasePixel checkoutSessionId="cs_test_noc" amountCents={100} currency="usd" />,
    );
    expect(oaiq).not.toHaveBeenCalled();
    expect(localStorage.getItem("matio:oaiq:purchase:cs_test_noc")).toBeNull();
  });
  ...
  it("never sends a negative or fractional amount", () => {
    ...
    window.oaiq = oaiq as unknown as NonNullable<typeof window.oaiq>;
    render(
      <PurchasePixel checkoutSessionId="cs_test_amt" amountCents={99.6} currency="eur" />,
    );
