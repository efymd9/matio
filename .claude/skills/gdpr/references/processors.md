...
| **Stripe Payments Europe Ltd** (SPEL, Ирландия) | платежи: Customer, billing address, карта (токен), инвойсы, `metadata` подписки (CAPI-снимок `_fbp`/`_fbc`/сырой IP/UA — только до `Purchase`, затем стирается, #165; отметка принятия отказа от 14-дневного права `tos_accepted_at`/`tos_version` — хранится постоянно, #210) | ЕС; передачи Stripe, LLC (US) и аффилиатам | авто (DPA «forms part of the Agreement»; контрагент вне Америк — SPEL) | DPF / EEA SCC / UK IDTA — по выбору Stripe | https://stripe.com/legal/dpa — 18.11.2025 |
