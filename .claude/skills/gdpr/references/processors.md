...
| **Stripe Payments Europe Ltd** (SPEL, Ирландия) | платежи: Customer, billing address, карта (токен), инвойсы, `metadata` подписки (CAPI-снимок `_fbp`/`_fbc`/сырой IP/UA — только до `Purchase`, затем стирается, #165) | ЕС; передачи Stripe, LLC (US) и аффилиатам | авто (DPA «forms part of the Agreement»; контрагент вне Америк — SPEL) | DPF / EEA SCC / UK IDTA — по выбору Stripe | https://stripe.com/legal/dpa — 18.11.2025 |
