...
export default defineConfig({
  ...
  test: {
    coverage: {
      ...
      // → 33.3 (#163: экспорт данных субъекта — lib/user-export.ts,
      //   lib/user-export-db.ts и вынесенный HogQL-транспорт покрыты
      //   целиком, лог-аудит +2 кейса; замер поверх слитого main (#186
      //   влит) — см. числа в PR; то же правило «max(main, локальный − 0.7)»,
      //   поднять до floor(CI, 0.1) по логу первого прогона CI).
      thresholds: {
        lines: 32.7,
        functions: 27.1,
        branches: 27.2,
