...
export default defineConfig({
  ...
  test: {
    coverage: {
      ...
      // → 33.4 (#163: экспорт данных субъекта — lib/user-export.ts,
      //   lib/user-export-db.ts и вынесенный HogQL-транспорт покрыты
      //   целиком, лог-аудит +2 кейса; 34.10/28.34/28.54/33.70 по прогону
      //   поверх слитого main (a5697df2, #186 влит). То же правило
      //   «max(main, локальный − 0.7)»; поднять до floor(CI, 0.1) по логу
      //   первого прогона CI).
      thresholds: {
        lines: 33.4,
        functions: 27.6,
