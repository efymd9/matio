...
export default defineConfig({
  ...
  test: {
    coverage: {
      ...
      // → 32.7 (#163: экспорт данных субъекта — lib/user-export.ts,
      //   lib/user-export-db.ts и вынесенный HogQL-транспорт покрыты
      //   целиком, лог-аудит +2 кейса; 33.45/27.86/27.98/33.05 локально.
      //   Полы = локальный замер − 0.7 п.п. дрейфа darwin→CI, измеренного
      //   на #183; поднять до floor(CI, 0.1) по логу первого прогона CI).
      thresholds: {
        lines: 32.7,
        functions: 27.1,
        branches: 27.2,
